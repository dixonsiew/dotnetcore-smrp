...
namespace smrp.Controllers.Report
{
    ...
    public class MasterPD102Controller : ControllerBase
    {
        ...
        public MasterPD102Controller(RsConnection rsconn, IMongoClient cli, ILogger<MasterPD102Controller> logger, ConfigService cfs, CommonSetupService cs, ReportService rs)
        {
            rscon = rsconn;
            client = cli;
            configService = cfs;
            ...
            [FromQuery(Name = "dateto")] string dateto = "")
        {
            ...
            foreach (var d in ls)
            {
                var person = new Dictionary<string, object>()
                {
                    ...
                    { "height",                    Helper.GetNum(Helper.GetStr(d["PERSON_HEIGHT"])) },
                    { "weight",                    Helper.GetNum(Helper.GetStr(d["PERSON_WEIGHT"])) },
                };
                ...
                var m = new Dictionary<string, object>()
                {
                    ...
                    { "refDisciplineCode",                await reportService.RefDisciplineCode1(d) },
                    { "refSpecialityCode",                await reportService.RefDisciplineCode1(d) },
                    { "refSubSpecialityCode",             await reportService.RefDisciplineCode1(d) },
                    { "refWardClassCode",                 await reportService.RefWardClassCode(d) },
                    { "refWardCategoryCode",              "00" },
                    { "gravida",                          Helper.GetStr(d["GRAVIDA"]) },
                    { "para",                             Helper.GetStr(d["PARITY"]) },
                    { "periodOfGestationDay",             Helper.GetNumber(Helper.GetStr(d["GESTATION_PERIOD"])) * 7 },
                    { "periodOfGestationWeek",            Helper.GetStr(d["GESTATION_PERIOD"]) },
                    { "isMotherAlive",                    d["ISMOTHERALIVE"] },
                    { "refAntenatalCareCode",             $"{d["REFANTENATALCARECODE"]}" },
                    { "refLabourStatusCode",              $"{d["LABOUR_METHOD"]}" },
                    { "labourDateTime",                   $"{d["DELIVERY_DATE"]} 00:00:00" },
                    { "bornBeforeArrival",                "N/A" },
                    { "isBabyAlive",                      Helper.GetStr(d["RESULT_OF_BIRTH"]) },
                    { "refLabourTypeCode",                "N/A" },
                    { "refLabourModeCode",                await reportService.RefLabourModeCode(d) },
                    { "refGenderCode1",                   await reportService.RefGenderCode1(d) },
                    { "birthWeight",                      Helper.GetNum(Helper.GetStr(d["WEIGHT"])) },
                    { "birthLength",                      Helper.GetNum(Helper.GetStr(d["LENGTH"])) },
            ...
            var filename = $"{ds1}_{ds2}_PD102.json";
        ...
        private IMongoCollection<BsonDocument> GetCollection(IMongoClient cli, string username)
        {
            var db = GetDb(cli);
